 ...
 */
export async function validateAndCreate<T>(body: any, ClassType: new () => T): Promise<T> {
...
export async function validateAndCreate2<T>(body: any, ClassType: new () => T): Promise<T | null> {
  const instance: any = new ClassType()
  Object.assign(instance, body)
  const arrayErrors = await validate(instance, ValidatorOptions)
  ...
  if (arrayErrors.length === 0) {
    ...
    const errorMessages = arrayErrors
      .map(error => {
